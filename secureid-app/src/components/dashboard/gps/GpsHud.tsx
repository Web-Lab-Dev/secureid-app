...
export function GpsHud({ distance, activeZones }: GpsHudProps) {
  return (
    <>
      ...
      <motion.div
        className="absolute left-3 top-3 z-10 flex max-w-[calc(100%-8.5rem)] flex-col gap-2 sm:left-4 sm:top-4 sm:max-w-none"
        initial={{ opacity: 0, x: -20 }}
        animate={{ opacity: 1, x: 0 }}
        transition={{ delay: 0.3 }}
      >
        {/* Badge LIVE */}
        <div className="flex w-fit items-center gap-2 rounded-full bg-black/70 px-3 py-2 backdrop-blur-md">
        ...
        <motion.div
          className={`flex w-fit max-w-full items-center gap-2 rounded-full px-3 py-2 backdrop-blur-md ${
      ...
      >
        <div className="rounded-lg bg-white px-3 py-2 shadow-xl sm:px-4">
